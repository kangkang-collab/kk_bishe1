...
class TransfuserBackbone(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        class Cfg: pass
        config = Cfg()
        config.perception_output_features = perception_output_features
        config.bev_features_chanels=bev_features_chanels
        config.bev_upsample_factor=bev_upsample_factor
        config.img_vert_anchors = img_vert_anchors
        config.img_horz_anchors = img_horz_anchors
        config.lidar_vert_anchors = lidar_vert_anchors
        config.lidar_horz_anchors = lidar_horz_anchors
        config.seq_len = seq_len
        config.n_head = n_head
        config.block_exp = block_exp
        config.n_layer = n_layer
        config.embd_pdrop = embd_pdrop
        config.attn_pdrop = attn_pdrop
        config.resid_pdrop = resid_pdrop
        config.gpt_linear_layer_init_mean = gpt_linear_layer_init_mean
        config.gpt_linear_layer_init_std  = gpt_linear_layer_init_std
        config.gpt_layer_norm_init_weight = gpt_layer_norm_init_weight
        config.use_point_pillars = use_point_pillars
        config.lidar_seq_len = lidar_seq_len
        ...
        self.lidar_encoder = LidarEncoder(architecture=lidar_architecture, in_channels=3,
                                          out_features=self.config.perception_output_features)
    ...
    def forward(self, image, lidar, velocity=None):
        '''
        ...
        '''
        # print(f"Input image shape: {image.shape}") # 48,3,256,704
        # print(f"Input lidar shape: {lidar.shape if lidar is not None else 'None'}") # 8,4,256,704
        
        if self.image_encoder.normalize:
            image_tensor = normalize_imagenet(image)
        else:
            image_tensor = image
        # repeat lidar for each camera
        # lidar = lidar.repeat(6, 1, 1, 1)
        # lidar = lidar.unsqueeze(1).repeat(1, 6, 1, 1, 1)
        # lidar = lidar.reshape(48, 4, 256, 704)
        # print(f"Input image shape: {image.shape}") # 48,3,256,704
        # print(f"Input lidar shape: {lidar.shape if lidar is not None else 'None'}") # 8,4,256,704
        B = lidar.shape[0]
        lidar = lidar.reshape(B * 6, 3, 256, 704)
        ...
        # print(f"Before addition - image_features shape: {image_features.shape}")
        # print(f"Before addition - image_features_layer1 shape: {image_features_layer1.shape}")
...
class GPT(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor, velocity=None):
        """
        Args:
            image_tensor (tensor): B*4*seq_len, C, H, W (48,64,5,22)
            lidar_tensor (tensor): B*seq_len, C, H, W (48,64,8,8)
        ...
        # n_embd = lidar_tensor.shape(2)
        assert self.seq_len == 1
        print('image_tensor:',image_tensor.shape)
        print('lidar_tensor:',lidar_tensor.shape)
        ...
        expected_len = self.pos_emb.shape[1]
        current_len = token_embeddings.shape[1]

        if current_len < expected_len:
            pad = expected_len - current_len
            last_token = token_embeddings[:, -1:, :].repeat(1, pad, 1)
            token_embeddings = torch.cat([token_embeddings, last_token], dim=1)

        elif current_len > expected_len:
            token_embeddings = token_embeddings[:, :expected_len, :]
...
class ImageCNN(nn.Module):
    ...
    def __init__(self, architecture, normalize=True, out_features=512):
        ...
        ckpt_path = "/home/itslab/kk/SparseDrive/ckpt/resnet34-43635321.pth"
